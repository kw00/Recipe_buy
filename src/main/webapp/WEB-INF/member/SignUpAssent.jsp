...
본 약관은 서비스 이용자가 주식회사 RecipeBuy(이하 “회사”라 합니다)가 제공하는 온라인상의 인터넷 서비스(이하 “서비스”라고 하며, 접속 가능한 유∙무선 단말기의 종류와는 상관없이 이용 가능한 “회사”가 제공하는 모든 “서비스”를 의미합니다. 이하 같습니다)에 회원으로 가입하고 이를 이용함에 있어 회사와 회원(본 약관에 동의하고 회원등록을 완료한 서비스 이용자를 말합니다. 이하 “회원”이라고 합니다)의 권리•의무 및 책임사항을 규정함을 목적으로 합니다.
...
<script>
	function check() {
		...
		if ($('input:radio[name="memberInfoYn"]:checked').val() != 'Y') {
			alert('개인정보 수집 및 이용에 동의해주세요');
			$("input:radio[name=memberInfoYn]").eq(0).focus();
			return false;
		}
	}
</script>
